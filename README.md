<h1 align="center">👋 Hi, I'm <strong>Tapas Adhikary</strong></h1>
<h3 align="center">Demand-Stack Engineer • AI Builder • Tech Educator, tapaScript • Speaker • CEO, CreoWis</h3>

<p align="center">
  📺 YouTube(<a href="https://www.youtube.com/@tapasadhikary">English</a> | <a href="https://www.youtube.com/@tapascript-bangla">Bangla</a>) •
  <a href="https://www.tapascript.io">🌐 Website</a> •
  <a href="https://twitter.com/tapasadhikary">🐦 Twitter</a> •
  <a href="https://linkedin.com/in/tapasadhikary">💼 LinkedIn</a>
</p>


<a href="https://youtube.com/tapasadhikary" target="_blank"><img src="./images/YT Fusion - LinkedIn.png" alt="GitRoll Profile Badge"/></a>


## 🚀 About Me

I’m **Tapas Adhikary**, a demand-stack engineer with decades of experience in building real-world software products. I love simplifying complex technologies, whether through code, videos, workshops, or communities.

I work at the intersection of **JavaScript, AI, architectural thinking, and developer education**, helping developers build confidence, ship better products, and grow faster.

## 🧠 What I Do

- 🎥 Teach developers through **YouTube, blogs, and live workshops**
- 🏗️ Build **full-stack web & AI products** using JavaScript, TypeScript, React, Next.js, Node.js  
- 🤖 Develop **AI-powered tools, agents, and MCP-based integrations**    
- 🎤 Speak at **React Summit, React Nexus, meetups, dev conferences**  
- 🌱 Mentor developers, run tech communities, and guide career growth  
- 🧪 Advocate for **testing, debugging, and writing maintainable software**  
- 🧵 Share insights on **engineering, leadership, and the craft of coding**

## 🏢 CEO & Co-Founder — **CreoWis Technologies**

I lead **CreoWis Technologies**, a global software engineering company specializing in:

- AI product development  
- Agentic system design  
- Web & mobile app development  
- UX design & research
- Architecture as a Service (AaaS)
- DevOps & cloud infrastructure
- App modernization & MVP development
- Developer upskilling & tech training

At CreoWis, our mission is to **build high-quality digital products**, partner closely with founders and enterprises, and bring engineering excellence to every engagement.

🌐 **Website:** https://creowis.com

## 📚 My Journey as an Educator (English & Bangla)

Teaching is one of the most rewarding parts of my career. Over the years, I’ve built a strong developer community on **YouTube**, where I simplify complex engineering topics with storytelling, real-world demos, and practical guidance.

### English Channel — Helping Developers Grow Globally  
On my English YouTube channel, I teach:

- JavaScript fundamentals & deep dives  
- Modern React & performance patterns  
- Full-stack development with real projects
- Engineering productivity, debugging, and architecture  
- AI concepts, LLMs, agents, MCP & practical AI apps(Coming in January 2026)

👉 Check it out: https://www.youtube.com/@tapasadhikary

My goal:  
**Make developers confident by showing *how things actually work under the hood*.**

### Bangla Channel — Making Tech Accessible for Everyone  
On my Bangla channel, I focus on:

- Explaining programming concepts in simple Bangla  
- Motivating aspiring developers  
- Teaching fundamentals of software development, logic & mindset  
- Sharing relatable stories about career growth and struggles  
- Supporting learners from non-English backgrounds

👉 Check it out: https://www.youtube.com/@tapascript-bangla

This is my way of giving back to the community I grew up in —  
**removing language barriers so anyone can succeed in tech.**

### 🌱 The Mission Behind My Teaching  
No matter the language, my teaching philosophy remains the same:

- Teach with clarity  
- Use real examples  
- Share practical industry insights  
- Build confidence, not confusion  
- Help developers learn faster and smarter  

Across both channels, my mission is to **empower developers to build, learn, and believe in themselves.**

## 🌟 Founder of **ReactPlay**

I am the creator of **ReactPlay**, an open-source developer community where people learn React by building real-world apps.

- 🚀 100+ Plays  
- 🌍 Builders across the globe  
- 🏆 Recognized at **React Summit**  
- 🎓 Helping thousands of developers learn by doing  

👉 Check it out: https://reactplay.io

## 🎤 Speaking & Workshops

I speak at meetups, conferences, and developer communities.

### Notable Talks
- **CityJS India 2025** – Scalability
- **React Nexus 2025** – Next.js Caching Strategies  
- **React Summit** – React Compiler(Berlin, 2024), Next.js Performance(NY, 2025)  
- **Community Meetups** – Testing, JavaScript performance, AI topics  
- **Corporate Training** – React, JS, Testing, Architecture, AI for Devs  

I love sharing learnings from real-world engineering and helping teams level up.

## 🧰 Skills & Tools

- **Frontend:** React, Next.js, TailwindCSS  
- **Backend:** Node.js, Express, PostgreSQL  
- **AI:** LLMs, OpenAI APIs, Agents, MCP  
- **DevOps:** Docker, CI/CD, AWS  
- **Testing:** Nightwatch.js, Playwright  
- **Languages:** JavaScript, TypeScript  
- **Design:** Figma, UX Principles  

## 📝 Latest Content & Writing

I regularly share engineering content around:

- JavaScript performance  
- React patterns  
- Testing & debugging  
- AI concepts & terminologies  
- Career growth for developers  

You can explore more at: **https://www.freecodecamp.org/news/author/atapas/**

## 💬 Let's Connect

If you want to collaborate on building products, AI solutions, or want me to speak at your event:

📧 Email: tapas.adhikary@gmail.com  
🌐 Website: https://www.tapascript.io  
💼 LinkedIn: https://linkedin.com/in/tapasadhikary

## 🟢 Fun Fact

I strongly believe:  
> **“If you help the community grow, you grow automatically.”**

## ⭐ Support My Work

If my open-source or content work has helped you, consider giving a ⭐ to my projects or supporting me through [GitHub Sponsors](https://github.com/sponsors/atapas).
